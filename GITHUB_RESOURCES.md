...
**Companion file:** [`resources/mcp-registry.json`](resources/mcp-registry.json) — the machine-readable seed registry for the planned MCP Discover tab (feature #23 in `NEW_FEATURES_TODO.md`). Only verification-confirmed servers are included.
...
- **[deroproject/derohe](https://github.com/deroproject/derohe)** — the DERO homomorphic-encryption chain itself. Custom "RESEARCH LICENSE" v1.1.2 **forbidding commercial use** — cannot be vendored into MIT Dero Hive; interoperate at arm's length via its stable local JSON-RPC (daemon `:10102`, wallet `:10103` mainnet; `:40402/:40403` testnet — the surfaces `dero-mcp-server` already targets). Go, official Windows binaries, Release142 (Aug 2025), 222★. Contains the reference **DVM** implementation in `/dvm`.
- **[civilware/tela](https://github.com/civilware/tela)** — TELA decentralized-web standard: on-chain app files (TELA-INDEX-1 / TELA-DOC-1 contracts, DVM-BASIC), executed locally; XSWD wallet connectivity. **MIT**, Go, pushed 2026-05-30, 20★, no releases. Complements the bundled `tela-*` skills; a TELA MCP tool (serve/install/update TELA apps) would be a distinctive Dero Hive feature.
- **[civilware/Gnomon](https://github.com/civilware/Gnomon)** — decentralized DERO indexer with queryable SC/transaction data (REST endpoints, `listsc` CLI). **MIT**, Go. **Dormant** (last main commit Nov 2023, 22★); build-from-source only. Still the obvious structured chain-data source for an agent.
...
- **Why it matters here:** this is the missing implementation piece for the Agent-tab concept — embed it in the renderer and an "Agent mode" can operate Dero Hive's own UI (navigate panels, fill the composer, manage conversations — or drive a future wallet tab) using the user's already-configured provider. Goals served: ship-in-app + the wallet×agent design direction.
...
2. Hunt specifically for wallet apps with an integrated AI Agent tab (the Ripley Terminal pattern) — no verified example exists; eliza is the nearest. Prototyping with page-agent inside Dero Hive may be faster than finding prior art.

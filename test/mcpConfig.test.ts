...
const { McpManager, buildMcpChildEnv, validateMcpConfig } = await import('../src/main/mcp/manager');
...
describe('MCP config validation', () => {
  test('inherits the allowed DERO daemon URL and lets server config override it', () => {
    const previous = process.env.DERO_DAEMON_URL;
    process.env.DERO_DAEMON_URL = 'http://192.168.2.251:10102';
    try {
      expect(buildMcpChildEnv().DERO_DAEMON_URL).toBe('http://192.168.2.251:10102');
      expect(buildMcpChildEnv({ DERO_DAEMON_URL: 'http://127.0.0.1:10102' }).DERO_DAEMON_URL)
        .toBe('http://127.0.0.1:10102');
    } finally {
      if (previous === undefined) delete process.env.DERO_DAEMON_URL;
      else process.env.DERO_DAEMON_URL = previous;
    }
  });
